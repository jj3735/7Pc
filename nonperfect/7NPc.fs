// THE 7NPC COMPILER (aka 7c nonperfect compiler)
module Nonperfect_
open System
open Fussless
...
type LLVMCompiler = 
  ...
  // translates an lltype to an LLVMtype
  member this.translate_lltype(t:lltype) =
    match t with
    | LLint -> Basic("i32")
    | LLfloat -> Basic("double")
    | LLvar(x) ->
      let potential_entry = this.symbol_table.get_entry(x)
      let mutable ll_type = LLuntypable

      if (isSome potential_entry) then
        ll_type <- potential_entry.Value.typeof
        
      // points to an int address
      Pointer(this.translate_lltype(ll_type))
    | LLunknown -> Basic("i32") // TODO: this is temp
    | LLfun(args,return_type) -> this.translate_lltype(return_type)
    | LLunit -> Void_t
    // NOTE: LLstring, LList, LLunknown and LLtuple may have to be
    // implemented... LLtuple may be translated first type??
    // this is for LLuntypable...
    | _ ->
      printfn "translate_lltype: tried to translate type %A which isn't supported.." t
      Void_t
  ...
  member this.expr_LLVMtype(expression:expr) =
    ...
    | Binop(op,_,_) | Uniop(op,_) ->
      // gets [type] from [op]_[type]
      let separator = op.IndexOf("_")
      let mutable ll_type = LLuntypable

      let expr_type = op.[separator+1..op.Length-1]
      match expr_type with
      | "LLint" -> (ll_type <- LLint)
      | "LLfloat" -> (ll_type <- LLfloat)
      | "LLstring" -> (ll_type <- LLstring)
      | "LLunit" -> (ll_type <- LLunit)
      | "LLunknown" -> (ll_type <- LLunknown)
      | "LLuntypable" -> (ll_type <- LLuntypable)
      // NOTE: LLvar, LList, LLtuple and LLfun might be needed..
      | _ ->
        printfn "expr_LLVMtype (binop/uniop): tried to get ll_type from string %A, which isn't supported.." expr_type

      this.translate_lltype(ll_type)
    | Var(x) ->
      let potential_entry = this.symbol_table.get_entry(x)
      if (isSome potential_entry) then
        let x_entry = potential_entry.Value
        this.translate_lltype(x_entry.typeof)
      else Void_t
    | Beginseq(list) | Sequence(list) ->
      let mutable mx = list.[list.Length-1].value
      this.translate_lltype(this.symbol_table.infer_type(&mx))
    | TypedLambda(_,given_return_type,_) ->
      // we can use the given return type since it's typechecked
      this.translate_lltype(given_return_type)
  ...
  member this.op_to_LLVMop(operation:expr) =
    ...
    | Binop(op,_,_) | Uniop(op,_) ->
      let separator = op.IndexOf("_")
      let mutable expr_op = ""
      let mutable operation_type_string = ""

      if (separator <> -1) then
        expr_op <- op.[0..separator-1]
        operation_type_string <- op.[separator+1..op.Length-1]
      else expr_op <- op

      let mutable is_float = false
      if (operation_type_string = "LLfloat") then is_float <- true
      ...
      | "^" -> "xor"
      // Uniop //
      | "not" -> "eq" // not is eq w/ 0.
      | "~" -> "fneg"
      // TODO: car & cdr for arrays
      | _ ->
        printfn "op_to_LLVMop (binop/uniop): could not get LLVMop from expr_op %A" expr_op
        ""
    | (_ as op) ->
  ...
  member this.compile_expr(expression:expr, func:LLVMFunction) =
    ...
    | Setq(Lbox(x),e) -> 
      ...
      if (isSome potential_entry) then
        ...
        let x_LLVMtype = this.translate_lltype(x_entry.typeof)
    ...
    | Sequence(list) ->
      if (list.Length = 0) then Novalue
      else if (list.Length = 1) then
        // a sequence returns the last thing compiled...
        this.compile_expr(list.[0].value,func)
      else
        // PSEUDOCODE //
        // since i don't have apply in my syntax (yet),
        // we check if the first value is a variable.
        // if it is, and it is a lambda, then we get
        ...
        let first_value = list.[0].value
        this.compile_expr(first_value,func) |> ignore

        match first_value with
        | (Var(s) as v) ->
          // this is an attempted function application. 
          // does the variable exist?
          let potential_entry = this.symbol_table.get_entry(s)
          if (isNone potential_entry) then Novalue
          else
            let v_entry = potential_entry.Value

            // checking if reg is needed for call inst later
            let mutable potential_reg = None
            match v_entry.typeof with
            | LLint | LLfloat | LLstring | LLvar(_) ->
              potential_reg <- Some(this.new_lID("func"))
            | LLfun(_,return_type) ->
              if (return_type<>LLunit) then
                potential_reg <- Some(this.new_lID("func"))
            | _ -> ()

            // is the variable an actual function?
            match v_entry.typeof with
            | LLfun(_,_) ->
              // getting function arguments //
              let mutable func_args = []

              for arg in list.[1..list.Length-1] do
                let i_dest_type = this.expr_LLVMtype(arg.value)
                ...
                | Var(x) ->
                  let mutable x_id = ""
                  let potential_entry =
                    this.symbol_table.get_entry(x)
                  if (isSome potential_entry) then
                    let x_gindex =
                      potential_entry.Value.gindex
                  ...
                  func.add_inst(load_inst)
                | _ ->
                  i_dest <- this.compile_expr(arg.value,func)

                func_args <- (i_dest_type,i_dest)::func_args
              // end for
              func_args <- List.rev func_args
              ...
              for var in free_vars do
                ...
                for f_arg in func.formal_args do
                  let (f_arg_type, f_arg_name) = f_arg
                  let f_arg_separator = f_arg_name.IndexOf("_")
                  let only_f_arg_name =
                    f_arg_name.[0..f_arg_separator-1]
                  if (only_v_name = only_f_arg_name) then
                    reg_name <- f_arg_name
              ...
              let all_args =
                List.append func_args fixed_free_vars

              let func_LLVMtype =
                this.translate_lltype(v_entry.typeof)
              let func_name =
                s + "_" + (string v_entry.gindex)

              let call_inst =
                Call(potential_reg,func_LLVMtype,[],func_name,
                     all_args)
              func.add_inst(call_inst)
            ...
            | _ ->
              ...
              printfn "attempting to call %A as a function when it is type %A..." s v_entry.typeof
              Novalue
            // v_entry.ast_rep match end
        | _ ->
          // this case should probably only be executed if
          // you're given a sequence of expressions instead,
          // i.e., the whole program. so we compile each expr...
          for e in list.[1..list.Length-2] do
            this.compile_expr(e.value,func) |> ignore

          // ...and return the last thing compiled.
          this.compile_expr(list.[list.Length-1].value,func)
        // first_value match end
    | Var(x) | TypedVar(_,x) ->
      let potential_entry = this.symbol_table.get_entry(x)
      ...
      if (isNone potential_entry) then Novalue
        ...
        let LLVM_x_type = this.translate_lltype(x_type)

        let x_id = x + "_" + (string x_entry.gindex)
        let new_x_id = this.new_lID(x_id)
        match x_type with
        | LLfun(_,_) -> Register(new_x_id)
        | _ ->
          let load_inst =
    ...
    | Define(Lbox(x),value) | TypedDefine(Lbox(_,x),value) ->
      ...
      let mutable free_var_set = this.symbol_table.find_closure()
    ...
    | Let((Lbox(s) as lbS),value,(Lbox(_) as lb_expr)) ->
      ...
      let var_LLVMtype = this.translate_lltype(var_entry.typeof)
  ...
  member this.compile_function(expression:expr, func:LLVMFunction) =
    ...
    | Define(
        Lbox(def_name),
        TypedLambda(lambda_vars,return_lltype,(Lbox(lbexpr) as lb))
      ) ->
      // restarting lindex
      let original_lindex = this.lindex
      this.lindex <- 0
      ...
      for (lvar_lltype,lvar_name) in lambda_vars do
        // getting the LLVMtype of var
        let var_LLVMtype = this.translate_lltype(lvar_lltype)
        ...
        if (isNone (this.symbol_table.get_entry(lvar_name))) then
          ...
          this.symbol_table.global_index <-
            this.symbol_table.global_index - 1
          this.symbol_table.add_entry(lvar_name,lvar_lltype,None)
      //// END FOR
      ...
      // adding free_var:<(string*int),lltype> to
      ...
      for var in free_var_set do
        ...
        let var_LLVMtype = this.translate_lltype(var.Value)
      ...
      let return_LLVMtype = this.translate_lltype(return_lltype)
      let some_new_func =
        { // LLVMfunction
          name        = def_name_id;
  ...
  member this.compile_program(mainseq:byref<expr>) =
    let program_type = this.symbol_table.infer_type(&mainseq);
    //printfn "DA AST NOW BE LIKE: %A" mainseq
    printfn "PROGRAM TYPE BE LIKE: %A" program_type
    if program_type = LLuntypable then
...
try program_name <-
  (Environment.GetCommandLineArgs() |> Array.tail |> Array.head)
with
  | :? System.ArgumentException ->
    printfn "You must input a file to compile!"
let program = System.IO.File.ReadAllText(program_name)

// generating program AST
let parser = make_parser();
let lexer = nonperfect_lexer<unit>(program)
...
let global_table_frame =
  { // table_frame
    ...
    parent_scope = None
    closure      = new SortedDictionary<(string*int),lltype>()
  };
let ST =
  { // SymbolTable
...
// compiling a test
//let mutable test = Sconst("Hello world!")
//compiler.compile_program(&test)

// compiling the program
let mutable p = program_AST.Value
let LLVM_program = compiler.compile_program(&p)
//printfn "\n\n\nCODE:\n%s" LLVM_program
